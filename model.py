...
class FBNet(nn.Module):
    ...
    def forward(self, input, target, temperature=5.0, theta_list=None):
        ...
        self.loss = self.ce * self._alpha * torch.log(self.lat_loss).pow(self._beta)
    ...
    def search(self, train_w_ds,
        ...
        for epoch in range(total_epoch):
            ...
            for step, (input, target) in enumerate(train_t_ds):
                ...
                self.save_theta('%s/%s_theta_epoch_%d.txt' %
                                (self.theta_result_path, self.save_theta_prefix, epoch + start_w_epoch))
            ...
            for step, (input, target) in enumerate(train_w_ds):
                self._step(input, target, epoch + start_w_epoch,
                           ...
                           lambda x, y: self.train_w(x, y, False))
                if step % 10 == 0:
                    torch.save(self._mod.state_dict(), '%s/%d_w.pt' % (self.checkpoints_path, epoch+start_w_epoch))
                self.w_sche.step()
